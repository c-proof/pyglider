...
sensorlist = str(example_dir / 'example-slocum-littleendian/dfo-maria997_sensors.txt')
binarydir = str(example_dir / 'example-slocum-littleendian/realtime_raw/') + '/'
rawdir_slocum = str(example_dir / 'example-slocum-littleendian/realtime_rawnc/') + '/'
deploymentyaml_slocum = str(example_dir / 'example-slocum-littleendian/deployment.yml')
l1tsdir = str(example_dir / 'example-slocum-littleendian/L0-timeseries-test/') + '/'
scisuffix = 'tbd'
glidersuffix = 'sbd'
...
slocum.merge_rawnc(rawdir_slocum, rawdir_slocum, deploymentyaml_slocum,
                   scisuffix=scisuffix, glidersuffix=glidersuffix)
outname_slocum_le = slocum.raw_to_timeseries(rawdir_slocum, l1tsdir, deploymentyaml_slocum,
                                            profile_filt_time=400, profile_min_time=100)
output_slocum_le = xr.open_dataset(outname_slocum_le)
# Open test data file
test_data_slocum_le = xr.open_dataset(library_dir / 'tests/results/example-slocum-littleendian/L0-timeseries/dfo-maria997-20220614.nc')
variables_slocum_le = list(output_slocum.variables)


def test_variables_slocum_littleendan():
...
@pytest.mark.parametrize("var", variables_slocum)
def test_example_slocum_littleendan(var):
    # Test that variables and coordinates match
    assert output_slocum_le[var].attrs == test_data_slocum_le[var].attrs
    if var not in ['time']:
        np.testing.assert_allclose(output_slocum_le[var].values, test_data_slocum_le[var].values)
    else:
...
def test_example_slocum_littleendan_metadata():
    # Test that attributes match. Have to remove creation and issue dates first
